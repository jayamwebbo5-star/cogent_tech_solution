var table_column = [
  ...
  },
  {
    ...
    render: function (data, type, row) {
      return `      
      <button data-pid="${row["web_book_id"]}" class="waves-effect waves-light btn btn-primary-light btn-sm edit-book" type="button" ><i class="fi fi-sr-attribution-pencil"></i> Edit</button>
          <button data-pid="${row["web_book_id"]}" class="waves-effect waves-light btn btn-danger-light btn-sm del-book " type="button" ><i class="fi fi-sr-trash"></i> Delete</button>
      `;
    },
  },
  //<button data-pid="${row["web_book_id"]}" class="waves-effect waves-light btn btn-success-light btn-sm view-book" type="button" ><i class="fas fa-eye"></i> View</button>
...
$(document).on("click", ".edit-book", function () {
  ...
  if (pid > 0) {
    var result = common.ajax_fech("getBook", { web_book_id: pid });

    if (result["code"] == 200 && result["data"].length != 0) {
      $(".book-form input[name=web_book_id]").val(
        result["data"][0]["web_book_id"]
      );
      $(".book-form input[name=web_title]").val(
        result["data"][0]["web_title"]
      );
      $(".book-form input[name=web_description]").val(
        result["data"][0]["web_description"]
      );
        $(".book-form input[name=display_order]").val(
        result["data"][0]["display_order"]
      );
      $(".book-form input[name=web_url_1]").val(result["data"][0]["web_url_1"]);
      $(".book-form input[name=web_url_2]").val(result["data"][0]["web_url_2"]);
        $(".book-form input[name=web_url_3]").val(result["data"][0]["web_url_3"]);
      $(".book-form textarea[name=web_content]").val(
...
$(".book-form").on("submit", function (event) {
  ...
  common
    .ajax_save_file("saveBook", formData)
  ...
  swal
    ...
    .then((result) => {
      if (result.isConfirmed) {
        common
          .ajax_save("saveBook", update_data)
          .then(function (response) {
            if (response.code == 200) {
