var table_column = [
    ...
    },
    {
        ...
        render: function (data, type, row) {
            return `      
      <button data-pid="${row["web_blog_id"]}" class="waves-effect waves-light btn btn-primary-light btn-sm edit-blog" type="button" ><i class="fi fi-sr-attribution-pencil"></i> Edit</button>
          <button data-pid="${row["web_blog_id"]}" class="waves-effect waves-light btn btn-danger-light btn-sm del-blog " type="button" ><i class="fi fi-sr-trash"></i> Delete</button>
      `;
        },
    },
];
...
$(document).on("click", ".edit-blog", function () {
    ...
    $(".blog-form input[name=web_blog_id]").val(pid);
    var result = common.ajax_fech("getBlog", {web_blog_id: pid});
    let last_count = (pid > 0) ? Number(result["last_count"]) : (Number(result["last_count"]) + 1);
    $("#order_count").text(last_count);
    $(".blog-form input[name=display_order]").data('last_count', last_count).val(last_count);
...
$(".blog-form").on("submit", function (event) {
    ...
    common
            .ajax_save_file("saveBlog", formData)
...
// Delete
$(document).on("click", ".del-blog", function () {
    var pid = $(this).data("pid");
    var update_data = {
        web_blog_id: pid,
        is_deleted: 1,
    ...
    swal
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    common
                            .ajax_save("saveBlog", update_data)
                            .then(function (response) {
                                if (response.code == 200) {
...
// Image
$("input[name=web_client_image]").on("change", function (event) {
    const file = event.target.files[0];
    const allowedExtensions = /(\.jpg|\.jpeg|\.png)$/i;
    const errorMsg = $("#photo-client-msg");
    if (file) {
        if (!allowedExtensions.test(file.name)) {
            errorMsg.text("Invalid file type! Only jpg, jpeg, and png are allowed.");
            $("#web_client_image").attr("src", "");
            return;
        }
        // Clear error message and show preview
        errorMsg.text("");
        const reader = new FileReader();
        reader.onload = function (e) {
            $("#web_client_image").attr("src", e.target.result);
        };
        reader.readAsDataURL(file);
    } else {
        $("#web_client_image").attr("src", "");
...
$('.blog-form input[name=display_order]').on('input', function () {
