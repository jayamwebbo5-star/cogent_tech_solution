var table_column = [
  ...
  },
  {
    ...
    render: function (data, type, row) {
      return `      
      <button data-pid="${row["book_writer_id"]}" class="waves-effect waves-light btn btn-primary-light btn-sm edit-writer" type="button" ><i class="fi fi-sr-attribution-pencil"></i> Edit</button>
      <button data-pid="${row["book_writer_id"]}" class="waves-effect waves-light btn btn-danger-light btn-sm del-writer " type="button" ><i class="fi fi-sr-trash"></i> Delete</button>
      `;
    },
  },
  //<button data-pid="${row["book_writer_id"]}" class="waves-effect waves-light btn btn-success-light btn-sm view-writer" type="button" ><i class="fas fa-eye"></i> View</button>
...
// Add and Edit
$(document).on("click", ".edit-writer", function () {
  var vbk = "show";
  $(".writer-form")[0].reset();
  $("#web_image").attr("src", "");
  ...
  if (pid > 0) {
    var result = common.ajax_fech("getWriter", { book_writer_id: pid });

    if (result["code"] == 200 && result["data"].length != 0) {
      $(".writer-form input[name=book_writer_id]").val(
        result["data"][0]["book_writer_id"]
      );
      $(".writer-form input[name=writer_name]").val(
        result["data"][0]["writer_name"]
      ); 
...
// Delete
$(document).on("click", ".del-writer", function () {
  var pid = $(this).data("pid");
  var update_data = {
    book_writer_id: pid,
    is_deleted: 1,
  ...
  swal
    ...
    .then((result) => {
      if (result.isConfirmed) {
        common
          .ajax_save("saveWriter", update_data)
          .then(function (response) {
            if (response.code == 200) {
...
//status update
$(document).on("change", ".writer_status", function () {
  var pid = $(this).data("pid");
  var update_data = {
    book_writer_id: pid,
    for: "status",
