var table_column = [
    ...
    {
        data: "web_solution_id",
        width: "200px",
        className: "text-center my-btn position-relative",
        title: "Action",
        render: function (data, type, row) {
            return `      
      <button data-pid="${row["web_solution_id"]}" class="waves-effect waves-light btn btn-primary-light btn-sm edit-solution" type="button" ><i class="fi fi-sr-attribution-pencil"></i> Edit</button>
          <button data-pid="${row["web_solution_id"]}" class="waves-effect waves-light btn btn-danger-light btn-sm del-solution " type="button" ><i class="fi fi-sr-trash"></i> Delete</button>
      `;
        },
    },
];
...
$(document).on("click", ".edit-solution", function () {
    var vbk = "show";
    $(".solution-form")[0].reset();
    $("#web_image").attr("src", "");
    var pid = $(this).data("pid");
    $(".modal-name").text(pid > 0 ? "Edit" : "Add");
    $(".solution-form input[name=web_solution_id]").val(pid);
    var result = common.ajax_fech("getSolution", {web_solution_id: pid});
    let last_count = (pid > 0) ? Number(result["last_count"]) : (Number(result["last_count"]) + 1);
    $("#order_count").text(last_count);
    $(".solution-form input[name=display_order]").data('last_count', last_count).val(last_count);

    if (pid > 0) {
        if (result["code"] == 200 && result["data"].length != 0) {
            $(".solution-form input[name=web_solution_id]").val(
                    result["data"][0]["web_solution_id"]
                    );
            $(".solution-form input[name=web_title]").val(
                    result["data"][0]["web_title"]
                    );

            $(".solution-form input[name=display_order]").val(
                    result["data"][0]["display_order"]
                    );

          
            $(".solution-form input[name=web_icon]").val(
...
$(".solution-form").on("submit", function (event) {
    ...
    common
            .ajax_save_file("saveSolution", formData)
...
// Delete
$(document).on("click", ".del-solution", function () {
    var pid = $(this).data("pid");
    var update_data = {
        web_solution_id: pid,
        is_deleted: 1,
    ...
    swal
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    common
                            .ajax_save("saveSolution", update_data)
                            .then(function (response) {
                                if (response.code == 200) {
...
$('.solution-form input[name=display_order]').on('input', function () {
